          ...
          return (
            <div key={index} className="flex flex-col items-center justify-center">
              <Link href={`/viper/${link}`}>
                <div className="relative block overflow-auto rounded-md drop-shadow-sm hover:cursor-pointer">
                  <div className="absolute top-0 bottom-0 rounded-md  left-0 right-0 z-50 block w-full h-full bg-zinc-800 opacity-40 max-h-[145px]" />
                  <Image
                    quality={90}
                    src={`${image}`}
                    alt="folder picture"
              ...
              <h1 className="m-0 mt-2 text-xs font-semibold text-zinc-500">{title}</h1>
            </div>
          );
        })}
      </div>
